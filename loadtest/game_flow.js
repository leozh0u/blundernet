...
const scenarios = {
  steady: {
    ...
    maxVUs: Math.max(60, RATE * 25),
  },
  ramp: {
    executor: 'ramping-arrival-rate',
    startRate: 2,
    timeUnit: '1s',
    preAllocatedVUs: Math.max(30, PEAK * 4),
    maxVUs: Math.max(120, PEAK * 15),
    stages: [
      { target: Math.ceil(PEAK / 4), duration: '45s' },
      { target: Math.ceil(PEAK / 2), duration: '45s' },
      ...
      // could respond to it.
      { target: PEAK, duration: '6m' },
      { target: 2, duration: '30s' }, // drain
    ],
  },
}
